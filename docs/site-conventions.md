# Website conventions and workflow context

This neutral reference supplements `development.md` with durable site conventions, content workflows, analytics, design tokens, and deployment context. Agent-specific entry points should link here rather than duplicating it.
...
3. **Title selection.** Generate 7 title candidates per article (pillar and every cluster post), informed by real SERP/NeuronWriter competitor titles and the site's own established title conventions (see examples in house style below), and pick one recommendation with a stated reason. Do not default to the first idea; a title that names the mechanism (what the reader gets, which tools/entities it covers) reliably outperforms a vague curiosity hook for this ICP.
4. **NeuronWriter analysis (per seed).** Run a query for each article keyword (see API below). Capture `terms`, `ideas.topic_matrix` (importance-ranked questions), `ideas.content_questions`, `competitors` (the score to beat), and `metrics.word_count` (the length target).
5. **Research.** Mine web + Reddit + community automatically; reuse existing dumps in `src/pages/posts/_prompts/research-resources/` when present. Write a scored insight bank to `docs/research/blog/<topic>/`.
6. **Write.** Cover the NeuronWriter terms, use the `topic_matrix` questions as H2/FAQ, write answer-first passages, include at least one tactical takeaway (below), and match house style (below).
...
- Convert the markdown body to HTML for `/evaluate-content` and `/import-content` (Python `markdown` with the `tables` extension is available). Do NOT use a crude newline-to-`<br>` conversion, it produces no real `<h2>`/`<table>` tags and starves NeuronWriter's heading-weighted term matching, understating the true score by a wide margin (one page scored 35 with `<br>` conversion, 65 with the correct method, against the identical content).
- Watch for contaminated keywords: some "hubspot ..." keywords return SERP intent about HubSpot's own pricing/subscription rather than the customer use case (e.g. "hubspot renewal quote price increase", "turn off auto-renewal"). Keep the article on-brand for the ICP and accept a lower score, or retarget to a cleaner keyword. The same trap applies one level up: a broad-looking keyword basket (e.g. generic "contract management software" for an "energy" vertical pull) can carry zero real overlap with the ICP, check for the brand/product terms you'd expect (e.g. "hubspot") before trusting a keyword list's relevance, not just its volume.
- After any post-publish edit, re-score against the same query used originally, not a fresh one, to get a like-for-like before/after comparison.
...
- **Include at least one genuinely tactical, copy-paste takeaway per article** (a specific property schema, a scoring formula with real point weights, a working query/cohort-definition template, a decision scorecard), not just conceptual guidance ("build a workflow," "define your metrics"). This is what should make a reader think "I can implement this today," distinct from and in addition to hitting NeuronWriter's term coverage. Flag anything unverifiable (exact third-party API/query syntax) as a template to adapt rather than presenting it as guaranteed-working.
- Internal links: contextual, descriptive keyword anchors to other `/posts/...` pages; verify every link resolves to an existing file before building, and always include the trailing slash (`/posts/example/`, not `/posts/example`), the site's `trailingSlash: "always"` config makes a missing slash a real crawl-visibility bug, not just a style nit, confirmed via a page that got zero Search Console impressions for six weeks because its only two inbound links were missing it.
