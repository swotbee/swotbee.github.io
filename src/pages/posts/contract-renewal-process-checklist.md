...
|--------|-------|------------|
| T-120 | Kickoff | Renewal record created in the CRM with value, owner, and dates |
| T-120 to T-90 | Assess | Account health verdict: usage trend, support history, stakeholder map, risk tier |
| T-90 | Strategize | Written renewal plan: uplift target, term proposal, expansion angle, walk-away line |
| T-90 to T-60 | Engage | Renewal discussions opened with the customer; value recap or QBR delivered |
| T-60 to T-45 | Propose | Renewal quote sent with pricing, term, and any scope changes |
| T-45 to T-20 | Negotiate | Procurement and legal cycles absorbed; concessions traded, not given |
| T-20 to T-0 | Close | Signature collected, CRM updated, billing notified, next cycle scheduled |
...
|------|-------------|-----------|
| Renewal record creation | RevOps (via automation) | CS |
| Health assessment | CSM | Support, product |
| Renewal strategy and uplift target | CSM or account manager | Sales leadership, finance |
| Customer engagement and QBR | CSM | Original sales rep |
| Quote and proposal | Account manager or renewals manager | Finance (pricing approval) |
| Negotiation | Account manager | Legal, finance |
| Signature and close | Account manager | Legal |
| Billing handoff | Finance | RevOps |
| Post-renewal handoff and next-cycle setup | CSM | RevOps |

Three boundary rules that prevent most turf problems:

- **CS owns the relationship; a commercial owner closes the contract before it ends.** Asking a CSM to both advocate for the customer and push an 8 percent uplift creates a conflict many CSMs resolve by not pushing. If CSMs do own the number, train and comp them for it explicitly.
- **Finance is consulted before the quote, not after the signature.** Pricing approval at T-60 beats an invoice dispute at T+10.
- **Expansion follows the renewal owner.** If the account shows expansion signals, the renewal conversation carries them; splitting renewal and expansion into separate motions with separate owners doubles the customer's meeting load and halves your leverage.
...
**Phase 2: Assess (T-120 to T-90)**

- [ ] Usage/adoption trend reviewed (rising, flat, declining)
...
- [ ] Value summary drafted: outcomes delivered this term, with numbers, benchmarked against the previous contract
- [ ] Uplift justified in writing (usage growth, added features, market rate)
- [ ] Renewal objectives written down so the most important renewal decisions, including the walk-away line, are set before negotiation starts
- [ ] Multi-year or longer-term offer prepared as a trade for price protection
- [ ] Walk-away position and maximum concession approved by finance
- [ ] Red-tier accounts: save plan created and escalated to leadership

**Phase 4: Engage (T-90 to T-60)**

- [ ] Renewal conversation booked with the economic buyer (not only the day-to-day contact)
- [ ] Value recap or QBR delivered
- [ ] Renewal intent confirmed verbally; objections logged in CRM
- [ ] Timeline agreed with customer, including their procurement lead time

**Phase 5: Propose and negotiate (T-60 to T-20)**
...
- [ ] Signature collected and the renewed contract filed in your centralized contract storage repository, so once the contract is renewed nobody has to hunt for the PDF
- [ ] CRM updated: deal closed won, new contract start/end dates entered
...
Tier note for HubSpot teams: all of the above needs Sales Hub or Service Hub Professional or higher. The known native gap is the kickoff step itself: workflow-created deals do not carry line items and multi-year chains hit loop protection, so most teams automate deal creation with an app or custom code and use native workflows for everything downstream. Reporting on renewal patterns across the book of business, once the data is flowing, also surfaces which reps need coaching before the next cycle.

### Where AI-Powered Contract Tools Fit

AI-powered contract analysis and contract automation are genuinely useful at exactly one point in this process: backfill. If your contract data lives in a folder of signed PDFs, AI extraction can pull end dates, notice periods, auto-renew clauses, and contract terms into your CRM in hours instead of weeks. Many teams report that tools like this can help streamline your contract backfill work, and that it simplifies the contract renewal process at the data-entry stage; treat the output as a draft and verify every date, then let the deterministic workflows above run each renewal cycle. AI reads contracts well; it does not own renewals.

---

## Frequently Asked Questions About Contract Renewals

**What are the steps in the contract renewal process?**
Five phases: assess the account's health and stakeholders, set the renewal strategy (uplift, term, risks), engage the customer with a value-led conversation, propose and negotiate the new term, then close and hand off to billing while scheduling the next cycle. Each phase has a named owner and a deadline counted back from the contract end date, which is what separates well-run contract renewals from ad hoc ones.

**How long before expiration should the renewal process start?**
Start work at 120 days out for contracts with procurement or legal review, and no later than 90 days for standard B2B contracts. The first customer conversation should happen by 75 days out; starting inside 30 days reliably costs either the uplift or the renewal itself.
...
**What are the benefits and disadvantages of automatic contract renewals?**
